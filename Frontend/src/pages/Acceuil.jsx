...
import { REACT_APP_API_URL } from "../config/constants/env.config";

function Acceuil() {
	const [data, setData] = useState(null);

	// useEffect pour récupérer les données de l'API au chargement du composant
	useEffect(() => {
		// fonction asynchrone pour récupérer les données de l'API
		const fetchData = async () => {
			try {
				// appel à l'API avec le fetch (méthode GET par défaut)
				const response = await fetch(`${REACT_APP_API_URL}/mohcine`);

				// récupération des données en JSON (pour le traitement des données)
				const result = await response.json();

				// mise à jour du state pour afficher les données
				setData(result);
			} catch (error) {
